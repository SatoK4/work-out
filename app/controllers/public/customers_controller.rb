class Public::CustomersController < ApplicationController
  def whisper
    ...
    @whispers = @customer.whispers.page(params[:page]).per(15).order(created_at: :desc)
  end

  def whisper_bookmark
    @customer = Customer.find(current_customer.id)
    bookmarks = WhisperBookmark.where(customer_id: current_customer.id).order(created_at: :desc).pluck(:whisper_id)
  ...
  def workout
    @customer = Customer.find(params[:id])
    @workouts = @customer.workouts.page(params[:page]).per(10).order(created_at: :desc)
  end

  def workout_bookmark
    @customer = Customer.find(params[:id])
    bookmarks = WorkoutBookmark.where(customer_id: current_customer.id).order(created_at: :desc).pluck(:workout_id)
